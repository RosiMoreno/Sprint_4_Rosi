...
# Nivel 2
# Ejercicio 1 
-- Creación de una tabla con las columnas card_id, fecha y si han sido declinadas o no las transaccciones --
CREATE VIEW Ultimas3transacciones AS
...
SELECT * FROM ultimas3transacciones;

-- Creación de una tabla con el estado de las tarjetas --
# ACTIVA si no ha tenido ninguna operación denegada en las últimas tres transacciones
# DESACTIVA si las tres últimas transacciones han sido denegadas.
CREATE VIEW EstadoTarjetas AS
SELECT card_id,
	CASE WHEN Sum(declined) = 3 THEN 'DESACTIVA'
		 ELSE 'ACTIVA' END AS Estado_Tarjeta
FROM ultimas3transacciones
GROUP BY card_id;
SELECT * FROM EstadoTarjetas;

-- Creación de la vista EstadoTarjetas con una sola consulta --
#CREATE VIEW EstadoTarjetas AS
SELECT card_id,
	CASE WHEN Sum(declined) = 3 THEN 'DESACTIVA'
		 ELSE 'ACTIVA' END AS Estado_Tarjeta
